...
export class InteractivePoints
{
    ...
    )
    {
        ...
        const initialText = (translationKey && translations[lang]?.[translationKey]) ? translations[lang][translationKey] : text
        ...
        const textSize = context.measureText(initialText)
        ...
        context.fillText(initialText, textPaddingLeft + 1, height * 0.5 + textOffsetVertical)
        ...
        item.defaultText = text
        item.align = align
        item.label = label
        item.labelTexture = labelTexture
        item.canvas = canvas
        item.context = context
        item.font = font
        item.height = height
        item.textPaddingLeft = textPaddingLeft
        item.textPaddingRight = textPaddingRight
        item.textOffsetVertical = textOffsetVertical
    ...
    updateLabels()
    {
        ...
        for(const item of this.items)
        {
            if(item.translationKey && translations[lang]?.[item.translationKey])
            {
                item.updateLabel(translations[lang][item.translationKey])
            }
            else if(item.defaultText)
            {
                item.updateLabel(item.defaultText)
            }
        }
    }
}