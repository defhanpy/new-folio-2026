...
import fontData from '../../../../static/fonts/helvetiker_bold.typeface.json'
...
export class LandingArea extends Area {
    ...
    setLetters() {
        ...
        // Permanently disable and remove old BRUNO SIMON objects so RESET never revives them
        for (const reference of references) {
            const object = reference.userData?.object
            if (object) {
                this.game.objects.disable(object)
                for (const [key, val] of this.game.objects.list.entries()) {
                    if (val === object) {
        ...
        // Parse 3D Font synchronously
        const font = new FontLoader().parse(fontData)
        const text = 'DEFHANPY'

        const pStart = references[references.length - 1].position.clone()
        const pEnd = references[0].position.clone()
        const rotationQuat = references[0].quaternion.clone()

        // Center point and direction vector for clean, balanced spacing
        const center = pStart.clone().add(pEnd).multiplyScalar(0.5)
        const dir = pEnd.clone().sub(pStart).normalize()
        const letterSpacing = 1.35

        // Original Bruno Simon letter material (palette off-white cream #fff2e8)
        const letterMaterial = new MeshDefaultMaterial({
            colorNode: color('#fff2e8'),
            hasDropShadows: true,
            hasLightBounce: true,
            hasWater: false
        })
        ...
        for (let i = 0; i < text.length; i++) {
            const char = text[i]
            const offset = (i - (text.length - 1) * 0.5) * letterSpacing
            const pos = center.clone().add(dir.clone().multiplyScalar(offset))
            pos.y = pStart.y
            ...
            const object = this.game.objects.add(
                ...
                },
                {
                    ...
                    colliders: [
                        {
                            shape: 'cuboid',
                            parameters: [bSize.x * 0.48, bSize.y * 0.48, bSize.z * 0.48]
                        }
                    ]
                }
            )

            if (object.physical && object.physical.colliders[0]) {
