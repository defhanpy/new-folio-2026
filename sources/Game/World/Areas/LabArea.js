...
export class LabArea extends Area
{
    ...
    setImages()
    {
        ...
        this.images.getResourceAndLoad = (key) =>
        {
            ...
            if(!resource)
            {
                ...
                const loaderType = key && key.endsWith('.ktx') ? 'textureKtx' : 'texture'
    ...
    setUrl()
    {
        ...
        this.url.open = () =>
        {
            if(this.navigation.current.url && this.navigation.current.url !== '#')
            {
            ...
            for(const project of labData)
            {
                ...
                // Image
                {
                    ...
                    mini.startLoading = () =>
                    {
                        ...
                        const loaderType = project.imageMini && project.imageMini.endsWith('.ktx') ? 'textureKtx' : 'texture'
                        const loader = this.game.resourcesLoader.getLoader(loaderType)
                        ...
                        loader.load(
                            ...
                            (loadedTexture) =>
                            {
                                ...
                                const textureColor = texture(loadedTexture, vec2(uv().x, uv().y.oneMinus())).rgb
                ...
                // Text
                {
                    ...
                    const miniTitle = project.titleSmall || project.title.replace(/^([^-–—]+)\s*[-–—]\s*/, '')
                    textCanvas.updateText(miniTitle)
