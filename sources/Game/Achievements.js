...
export class Achievements
{
    ...
    setItems()
    {
        ...
        {
            const nameKey = `ach_${name}_${total}_name`
            const descKey = `ach_${name}_${total}_desc`
            const displayTitle = translations[lang]?.[nameKey] || title
            const displayDescription = translations[lang]?.[descKey] || description
            ...
            const html = /* html */`
                ...
                <div class="description">
                    <div class="text">${displayDescription}</div>
                    <div class="progress">
            ...
            achievement.titleElement = achievement.itemElement.querySelector('.title')
            achievement.descTextElement = achievement.itemElement.querySelector('.description .text')
            ...
            achievement.achieve = (_silent = true) =>
            {
                ...
                if(!_silent)
                {
                    ...
                    const notifTitle = translations[currentLang]?.[nameKey] || title
                    const notifDesc = translations[currentLang]?.[descKey] || description
                    const html = /* html */`
                        <div class="top">
                        ...
                        <div class="bottom">
                            <div class="description">${notifDesc}</div>
    ...
    setReset()
    {
        ...
        button.addEventListener('click', (event) =>
        {
            ...
            if(clickCount === 1)
            {
                button.textContent = translations[lang]?.achievements_reset_confirm || 'Are you sure?'
            }
            else if(clickCount === 2)
            {
                button.textContent = translations[lang]?.achievements_reset_definitely || 'Definitely?'
            }
            else if(clickCount === 3)
            {
                button.textContent = translations[lang]?.achievements_reset_done || 'Done!'
                clickCount = 0
                this.reset()
            }
        })
        ...
        button.addEventListener('mouseleave', (event) =>
        {
            ...
            const lang = getCurrentLanguage()
            button.textContent = translations[lang]?.achievements_reset || 'Reset achievements'
        })
    }
    ...
    updateTranslations()
    {
        ...
        if(this.resetButton)
        {
            this.resetButton.textContent = translations[lang]?.achievements_reset || 'Reset achievements'
        }
        ...
        this.groups.forEach(group =>
        {
            for(const item of group.items)
            {
                if(item.titleElement)
                    item.titleElement.textContent = translations[lang]?.[item.nameKey] || item.defaultTitle
                if(item.descTextElement)
                    item.descTextElement.innerHTML = translations[lang]?.[item.descKey] || item.defaultDescription
            }
        })
    }
